...
from server_admins.models import (Permission, Privileged, Role, Server,
                                  ServerPrivileged)
...
class Command(BaseCommand):
        ...
        with atomic():
            ...
            for config_priv in privilegeds:
                ...
                priv, _ = Privileged.objects.get_or_create(
                    ...
                    defaults={
                        'name': config_priv['comment']
                        or config_priv['steam_id'],
    ...
    def get_db_roles(self, roles):
        ...
        for config_role in roles:
            role = self.create_new_role_or_get_old(
                config_role['title'],
                [perm.strip() for perm
                 in config_role['permissions'].split(',')]
            )
    ...
    def create_new_role_or_get_old(self, role_name, permissions_names):
        ...
        old_role = None
        try:
            old_role = Role.objects.prefetch_related(
                'permissions'
            ).get(title=role_name)
    ...
    def create_role(self, permissions_names, role_name=None):
        ...
        while True:
            if role_name is None:
                role_name = question_with_confirmation(
                    'Введите название для новой роли'
                )
            new_role, created = Role.objects.get_or_create(title=role_name)
            if created:
                self.stdout.write(f'Роль {role_name} создана')
                break

            self.stdout.write(f'Роль {role_name} уже существует')
    ...
    def find_roles(self, file_text):
        ...
        Поиск ролей в файле
        """
        roles_regex = (r'(?m)^ *Group=(?P<title>\w+):'
                       r'(?P<permissions>[\w, ]+)(?: *(?://)*.*$)')
        roles = [_match.groupdict()
    ...
    def choose_server(self):
        ...
        while True:
            ...
            self.stdout.write(
                f'Выбран сервер - {servers[server_id]}'
            )
