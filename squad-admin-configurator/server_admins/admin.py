...
class ServerPrivilegedInline(admin.StackedInline):
    fields = (('is_active', 'creation_date', 'date_of_end'),
              'server', 'roles', 'privileged', 'comment')
...
class AdminServer(admin.ModelAdmin):
    list_display = ('title', 'description',
                    'is_active', 'type_of_distribution', 'local_filename')
    ...
    @admin.action(description='Обновить конфиг выбранных серверов',
                  permissions=["change"])
...
class AdminPrivileged(admin.ModelAdmin):
    ...
    fields = ('is_active', 'name', 'steam_id',
              'description', 'creation_date', 'date_of_end')
    readonly_fields = ('creation_date',)
    list_display = ('name', 'get_roles', 'is_active',
                    'date_of_end_view', 'creation_date')
    list_editable = ('is_active',)
    list_filter = (
        'is_active',
        'servers_roles',
    ...
    @admin.display(ordering='-date_of_end',
    ...
    def get_roles(self, obj):
        servers_roles = (obj.serverprivileged_set
