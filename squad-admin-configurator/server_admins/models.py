...
class Server(models.Model):
    ...
    TYPES_OF_DISTRIBUTION_WITH_API = [
        API,
        API_AND_LOCAL
    ]

    TYPES_OF_DISTRIBUTION_WITH_LOCAL = [
        LOCAL,
        API_AND_LOCAL
    ]
    ...
    def clean(self):
        ...
        if self.type_of_distribution in self.TYPES_OF_DISTRIBUTION_WITH_LOCAL:
            admins_filename_validator(
                self.local_filename,
                'local_filename'
            )

            if (Server.objects
                .filter(local_filename=self.local_filename)
                .exclude(pk=self.pk)
                    .exists()):
...
class Privileged(models.Model):
    ...
    steam_id = models.BigIntegerField('Steam ID',
                                      unique=True)
    ...
    creation_date = models.DateTimeField('Дата добавления',
                                         auto_now_add=True)

    date_of_end = models.DateTimeField('Общая дата окончания полномочий',
                                       blank=True,
                                       null=True)

    servers_roles = models.ManyToManyField(
        Server,
        through='ServerPrivileged',
        blank=True,
...
class ServerPrivileged(models.Model):
    ...
    privileged = models.ForeignKey(Privileged,
    ...
    creation_date = models.DateTimeField('Дата добавления',
                                         auto_now_add=True)
    date_of_end = models.DateTimeField('Дата окончания этой роли',
                                       blank=True,
                                       null=True)
