...
BM_MAX_DEVIATION = (settings.HMAC_VALIDATION
                    ['BATTLEMETRICS']
...
def generate_signarute_from_request(request, webhook_object):
    ...
    if webhook_object.request_sender == ReceivedWebhook.BATTLEMETRICS:
        ...
        deviation_timed = timedelta(
            seconds=BM_MAX_DEVIATION
        )
        ...
        return hmac.digest(webhook_object.hmac_secret_key.encode(),
                           f'{timestamp_text}.'.encode() + request.body,
                           webhook_object.hmac_hash_type).hex()
...
def validate_hmac_in_request(request, webhook_object):
    ...
    if not hmac.compare_digest(generated_hmac_signature,
                               request_hmac_signature):
        raise ValidationError(
