...
class WebhookLogAdmin(admin.ModelAdmin):
    fields = ('id', 'message', 'webhook_info', 'request_info', 'level',
              'creation_date', 'content_type', 'object_id',
              'content_object_link')
    readonly_fields = ('id', 'creation_date', 'content_object_link')
    list_display = ('level', 'message_display', 'creation_date',
                    'content_object_link')
    ...
    @admin.display(description='Объект записавший лог',
                   empty_value='-')
...
class RoleWebhookAdmin(admin.ModelAdmin):
    fieldsets = [
        (
            ...
            {
                'fields': [
                    'is_active', 'description', 'url', 'creation_date',
                    'servers', 'roles', ('unit_of_duration',
                                         'duration_until_end'),
...
class ServerUrlAdmin(admin.ModelAdmin):
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name == 'server':
            ...
            with_api_and_not_tied = (
                Q(type_of_distribution__in=Server.TYPES_OF_DISTRIBUTION_WITH_API)  # noqa: E501
                & Q(server_url=None)
            )

            kwargs['queryset'] = (
                Server.objects
                .filter(its_our_server | with_api_and_not_tied)
            )
